...
# StorageClass for gp3 (used by Loki PVCs)
...
module "addons_helm" {
  ...
  loki_values_yaml = templatefile("${path.module}/../../modules/addons-helm/values/loki.yaml.tmpl", {
...
module "rds_postgres" {
  ...
  # Restrict DB access to VPC CIDR (simple/dev-friendly). For production,
  # prefer security-groups-for-pods or a dedicated app SG.
